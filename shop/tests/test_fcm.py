...
class FCMFallbackDispatchTests(TestCase):
    ...
    def test_chat_fallback_uses_shop_channel_for_customer_message_to_shop(self, mock_send):
        ...
        self.assertEqual(first_call['data']['chat_id'], str(self.order.id))
        self.assertEqual(first_call['data']['order_id'], str(self.order.id))
        self.assertEqual(first_call['data']['order_number'], self.order.order_number)
        self.assertEqual(first_call['data']['shop_id'], str(self.shop.id))
        self.assertEqual(first_call['data']['shop_name'], self.shop.shop_name)
        self.assertEqual(first_call['data']['customer_id'], str(self.customer.id))
        self.assertEqual(first_call['data']['customer_name'], self.customer.name)
        self.assertEqual(first_call['data']['sender_id'], str(self.customer.id))
        self.assertEqual(first_call['data']['sender_type'], 'customer')
        self.assertEqual(first_call['data']['sender_name'], self.customer.name)
        self.assertEqual(first_call['data']['message_type'], 'text')
        self.assertEqual(first_call['data']['message_preview'], 'Hello from customer')
        self.assertEqual(first_call['data']['route'], '/chat')
        self.assertEqual(first_call['data']['click_action'], 'OPEN_CHAT')
    ...
    def test_chat_fallback_uses_delegate_sender_name_as_title_for_driver_customer_message(self, mock_send):
        ...
        self.assertEqual(target_record.user_type, 'customer')
        self.assertEqual(target_record.user_id, self.customer.id)
        self.assertEqual(mock_send.call_args.kwargs['title'], self.driver.name)
        self.assertEqual(mock_send.call_args.kwargs['data']['sender_name'], self.driver.name)
        self.assertEqual(mock_send.call_args.kwargs['data']['driver_name'], self.driver.name)
    ...
    def test_ring_fallback_targets_shop_owner_and_employees_for_shop_target(self, mock_send):
        ...
        for call in mock_send.call_args_list:
            self.assertEqual(call.kwargs['channel_id'], 'delivery_general')
            ...
            self.assertEqual(call.kwargs['sound'], 'default')
            self.assertEqual(call.kwargs['ios_sound'], 'default')
            self.assertEqual(call.kwargs['data']['sound'], 'default')
            self.assertEqual(call.kwargs['data']['channel_id'], 'delivery_general')
    ...
    def test_ring_fallback_targets_driver_with_urgent_payload_when_driver_has_no_active_socket(self, mock_send):
        ...
        self.assertEqual(mock_send.call_args.kwargs['data']['title'], 'Mohammed Eltony يتصل بك')
    ...
    def test_ring_fallback_targets_customer_with_driver_customer_chat_payload_when_driver_calls(self, mock_send):
        ...
        self.assertEqual(mock_send.call_args.kwargs['data']['call_id'], 'ring-customer-194')
        self.assertEqual(
            mock_send.call_args.kwargs['data']['conversation_id'],
            f'order_{self.order.id}_driver_customer',
        )
    ...
    @patch('shop.fcm_service.send_push_to_token_record', return_value={'success': True, 'invalid_token': False})
    def test_ring_fallback_targets_customer_with_shop_customer_chat_payload_when_shop_calls(self, mock_send):
        FCMDeviceToken.objects.create(
            user_type='customer',
            user_id=self.customer.id,
            device_id='customer-device-shop-chat-ring',
            platform='android',
            fcm_token='customer-token-shop-chat-ring',
            is_active=True,
        )
        ...
        summary = send_ring_push_fallback(
            self.order.id,
            {
                'ring_id': 'ring-shop-customer-194',
                'sender_type': 'shop_owner',
                'sender_id': self.shop.id,
                'sender_name': self.shop.shop_name,
                'targets': ['customer'],
                'chat_type': 'shop_customer',
            },
        )
    ...
    def test_ring_fallback_deduplicates_repeated_driver_target_entries(self, mock_send):
        FCMDeviceToken.objects.create(
            user_type='driver',
            user_id=self.driver.id,
            device_id='driver-device-dedup-target',
            platform='android',
            fcm_token='driver-token-dedup-target',
            is_active=True,
        )
        ...
        summary = send_ring_push_fallback(
            self.order.id,
            {
                'ring_id': 'ring-dedup-target',
                'sender_type': 'customer',
                'sender_name': 'Mohammed Eltony',
                'customer_name': 'Mohammed Eltony',
                'targets': ['driver', 'driver'],
                'chat_type': 'driver_customer',
            },
        )
    ...
    def test_driver_chat_call_ringing_fallback_targets_driver_when_store_calls_and_driver_offline(self, mock_send):
        ...
        self.assertEqual(mock_send.call_args.kwargs['data']['store_name'], self.shop.shop_name)
        self.assertEqual(mock_send.call_args.kwargs['data']['shop_name'], self.shop.shop_name)
        self.assertEqual(mock_send.call_args.kwargs['data']['title'], f'{self.shop.shop_name} يتصل بك')
    ...
    def test_customer_driver_chat_ring_payload_uses_delegate_shop_name(self):
        ...
        self.assertEqual(payload['sender_name'], self.driver.name)
        self.assertEqual(payload['driver_name'], self.driver.name)
        self.assertEqual(payload['driver']['name'], self.driver.name)
        self.assertEqual(payload['shop']['name'], self.shop.shop_name)
        self.assertEqual(payload['shop_logo_url'], 'https://example.com/media/shop_profiles/shop.jpg')
